...
# ----- Correlation -----
  ...
  # Code to generate correlated data
  data <- reactive({
...
# ----- Make example correlation scatterplot in R -----    
    ...
    output$weakplot_pos <- renderPlot({
      ggplot(cordata(), aes(x = V1, y = V2)) + 
        geom_point(color="pink", alpha=input$draw_n^1/input$draw_n^1.05) + 
        geom_smooth(method=lm, fill="cadetblue", color="dimgrey") +
        stat_ellipse(type = "norm", geom = "polygon", color=NA, fill="orange", alpha=.1)
      })
    
    output$medplot_pos <- renderPlot({
      ggplot(cordata(), aes(x = V1, y = V3)) + 
        geom_point(color="plum", alpha=input$draw_n^1/input$draw_n^1.05) + 
        geom_smooth(method=lm, fill="cadetblue", color="dimgrey") +
        stat_ellipse(type = "norm", geom = "polygon", color=NA, fill="orange", alpha=.1)
      })
    ...
    output$strongplot_pos <- renderPlot({
      ggplot(cordata(), aes(x = V2, y = V3)) + 
        geom_point(color="orchid", alpha=input$draw_n^1/input$draw_n^1.05) + 
        geom_smooth(method=lm, fill="cadetblue", color="dimgrey") +
        stat_ellipse(type = "norm", geom = "polygon", color=NA, fill="orange", alpha=.1)
      })
